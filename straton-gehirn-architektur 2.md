...
**Status:** Konzeptionell vollständig für die Schichten 1–7. Ein Bereich bewusst offen (siehe Kapitel 14).
**Gehört zusammen mit:** `straton-ui-spezifikation.md` — beide Dokumente ergänzen sich und sind einzeln unvollständig.
...
## 0.1 Änderungen gegenüber Version 1.0

Version 1.0 ist bereits implementiert. Dieses Kapitel sagt, was sich ändert und was unangetastet bleibt.

**Unverändert gültig:** Alle zwölf Invarianten. Alle Entscheidungen 1 bis 24. Gedächtnisaufbau, Propagation, Prüferausgabe, Planertyp, Kaltstart, Konsolidierung. Wer 1.0 gebaut hat, muss daran nichts zurückbauen.

**Fünf Ergänzungen, alle additiv:**

| Neu | Kapitel | Art des Eingriffs |
|---|---|---|
| Formatzuordnung zur Anwendungstiefe | 6.6 | neue Planerregel — bestimmt, welcher Aufgabentyp erzeugt wird |
| Zuständigkeitsgrenze Wiederholen gegen Pfad | 6.7 | neue Planerregel — war implizit, ist jetzt explizit |
| Vorratserzeugung für Wiederholungsabfragen | 7.1 | **begrenzte Ausnahme** von der Echtzeitregel |
| Erklärtexte als eigene Erzeugungsart | 7.3 | neue Generator-Ausgabe, erweitert den Prüfumfang des Kontrolleurs |
| Geltungsbereich des Musterkatalogs | 10 | Speicherort: pro Nutzer, nicht pro Lernpfad |

**Die einzige echte Abweichung von 1.0** ist die Vorratserzeugung in 7.1. Sie hebt „Echtzeit" nicht auf, sondern schränkt sie auf einen benannten Fall ein. Wer 7.1 in 1.0 wörtlich umgesetzt hat, muss dort nachziehen.

**Praktisch prüfen solltest du:** ob dein Musterkatalog an einen Lernpfad gebunden gespeichert wird. Falls ja, ist das die einzige Stelle mit Wanderungsbedarf.
...
### 6.6 Formatzuordnung zur Anwendungstiefe

Der Planer bestimmt nicht nur **welches** Konzept drankommt, sondern auch **welche Art Aufgabe**. Das Format ist keine Gestaltungsfrage und keine Nutzerwahl, sondern folgt aus der Anwendungstiefe, die bewegt werden soll.

| Anwendungstiefe | Zugelassene Formate | Evidenzstärke |
|---|---|---|
...
| **Übertragen** | eingekleidetes Szenario ohne Nennung des Konzepts, Fehlersuche in gegebener Lösung, Begründungsfrage | am höchsten, teuerste Bewertung |

**Warum das im Planer sitzt und nicht in der Oberfläche:** Die Formatwahl bestimmt, welche Evidenz überhaupt entsteht. Werden nie Übertragen-Aufgaben gestellt, bleibt die dritte Stufe im Lernerbild dauerhaft leer — unabhängig davon, wie gut der Nutzer ist. Gäbe man die Wahl an den Nutzer ab, wäre das Gehirn umgehbar.

**Regel für den Sitzungsaufbau:** innerhalb einer Sitzung steigt die Tiefe an. So entsteht pro Sitzung Evidenz auf mehreren Stufen statt fünfmal auf derselben.

Die Fehlersuche ist ein diagnostischer Sonderfall: sie prüft Übertragen und liefert zugleich besonders präzise Ursachen, weil der Nutzer selbst auf den Fehler zeigt.

### 6.7 Zuständigkeitsgrenze: Wiederholung gegen Pfad

Der Planer speist zwei getrennte Oberflächen. Die Grenze verläuft nach **Auslöser**, nicht nach Inhalt.

| | Wiederholung | Pfad |
|---|---|---|
| Auslöser | **Verfall** | **Fehler und Lücken** |
| Inhalt | was beherrscht wurde und wieder verloren geht | was noch nicht beherrscht wird |
| Anwendungstiefe | Erkennen | alle Stufen |
| Charakter | kurz, mechanisch | Erklärung, Ursachensuche, Einschübe |

**Folgerungen:**

- Ein nie gelerntes Konzept kann nicht verfallen und erscheint in der Wiederholung nie.
- Ein Konzept, das auf Anwenden- oder Übertragen-Ebene aufgefrischt werden muss, gehört in den Pfad — die Wiederholung ist auf Erkennen ausgelegt.
- Fehler landen nie in der Wiederholung. Ein Fehler zeigt, dass etwas fehlt; Fehlendes wird aufgebaut, nicht aufgefrischt.

**Übergänge in beide Richtungen:** Eine verpatzte Wiederholung senkt die Beherrschung und kann ein Konzept zurück in den Pfad befördern. Umgekehrt wandert ein Konzept erst dann in die Wiederholung, wenn es gefestigt ist und zu verfallen beginnt.
...
**Benannte Ausnahme — Wiederholungsabfragen (neu in 1.1):** Für den Wiederholungsstapel gilt Echtzeit **nicht**. Dort wird ein kleiner Vorrat pro Konzept vorgehalten und rotierend ausgespielt; er wird neu erzeugt, sobald sich im Lernerbild dieses Konzepts etwas geändert hat.

Begründung: Bei einer Lernsitzung mit fünf Aufgaben fällt Erzeugungszeit nicht auf, weil vorproduziert wird. Bei einem Stapel aus vielen kurzen Abfragen, den ein Nutzer im Zug durchklickt, ist Tempo das gesamte Produkterlebnis — dort wäre Erzeugung zwischen jeder Abfrage spürbar. Der Verlust an Momentbezug ist hier vertretbar, weil Wiederholungen auf Erkennen-Niveau kaum vom aktuellen Moment abhängen.

Diese Ausnahme gilt ausschliesslich für die Wiederholung. Alle Aufgaben im Pfad bleiben Echtzeit.
...
### 7.3 Erklärtexte als eigene Erzeugungsart (neu in 1.1)

Der Generator erzeugt nicht nur Aufgaben, sondern auch **Erklärungen**. Diese sind eine eigene Ausgabeart mit eigenen Regeln, kein Beiwerk der Rückmeldung.

**Grundsatz: Straton ist kein Lehrbuch.** Das Lehrbuch besitzt der Nutzer bereits — sein Material. Würde Straton den Stoff durchgängig in eigenen Worten erklären, konkurrierte es mit der Quelle, erzeugte Abweichungen und verschöbe die Ergänzungsgrenze aus Kapitel 3. Der Wert liegt im Diagnostizieren, Üben und im Erklären genau dort, wo es klemmt.

**Drei zugelassene Erklärstellen:**

| Stelle | Umfang | Bedingung |
|---|---|---|
| Einstieg vor der ersten Aufgabe | 3–5 Sätze | nur wenn Beherrschung und Sicherheit bei null |
| Rückmeldung nach dem Versuch | kurz, plus aufklappbarer Lösungsweg | immer |
| nach „weiss ich nicht" | vollständig | auf Anforderung des Nutzers |

Alles darüber hinaus gehört in den Chat, nicht in die Sitzung.

**Warum ein Einstieg nötig ist:** Kaltabfragen ohne jedes Vorwissen erzeugt Frust und keine verwertbare Evidenz — ein Nichtwissen ohne Vorwissen diagnostiziert nichts.

**Warum sonst erst nach dem Versuch:** Wer erst versucht und dann die Erklärung erhält, behält deutlich mehr als wer zuerst liest. Der Versuch erzeugt die Lücke, in die die Erklärung fällt. „Frage zuerst" ist kein Sparmodell, sondern die stärkere Reihenfolge.

**Pflichten:** Erklärtexte sind quellengebunden und tragen eine Herkunftsmarkierung (Invariante I4). Sie unterliegen dem Quellenabgleich durch den Kontrolleur (Invariante I5) wie jede Aufgabe. Ein halluzinierter Erklärtext ist gefährlicher als eine halluzinierte Aufgabe, weil der Nutzer ihn ungeprüft übernimmt.
...
### Geltungsbereich (neu in 1.1)

Der Musterkatalog wird **pro Nutzer** geführt, nicht pro Lernpfad. Muster aus allen Lernpfaden eines Nutzers laufen in denselben Katalog.

**Begründung:** Ohne pfadübergreifende Sammlung erreicht ein generisches Muster die Anzeigeschwelle unter Umständen nie, weil sich die Belege auf mehrere Pfade verteilen und keiner allein genügend Evidenz hat. Genau die generischen Muster sind aber die wertvollsten.

**Abgrenzung zu Entscheidung 4:** Wissensgraph und Lernerbild bleiben pro Lernpfad getrennt — die Konzepte von „Netzwerke" und „Steuern" haben nichts miteinander zu tun. Nur der Musterkatalog liegt eine Ebene höher, beim Nutzer.
...
| 25 | Formatzuordnung | folgt der Anwendungstiefe, Planerregel | Nutzerwahl, Zufall |
| 26 | Wiederholung gegen Pfad | Verfall füllt die Wiederholung, Fehler den Pfad | gemeinsamer Topf |
| 27 | Wiederholungsabfragen | Vorrat je Konzept, neu bei Änderung | Echtzeit auch dort |
| 28 | Erklärtexte | eigene Erzeugungsart, quellengebunden, geprüft | Teil der Rückmeldung, ungeprüft |
| 29 | Musterkatalog | pro Nutzer, pfadübergreifend | pro Lernpfad |
...
| **Schwellenwerte** | Ab welcher Beherrschung gilt ein Konzept als gefestigt, ab welchem Verfall als fällig, wie viel Evidenzgewicht löst Konsolidierung aus. Alles Zahlen, die am echten Nutzerverhalten kalibriert werden müssen. |
...
Erledigt seit 1.0: Produktionsformate (jetzt Kapitel 6.6), Erklärungen (Kapitel 7.3). Die Chatbündelung ist als Oberflächenablauf in die UI-Spezifikation gewandert.
