...
import type { PdfOutlineV1 } from './pdfSpec'
import { sanitizePdfFileName } from './pdfSpec'
...
/**
 * Erzeugt PDF-Bytes aus {@link PdfOutlineV1} (JSON-Gliederung).
 */
export async function buildPdfFromOutline(outline: PdfOutlineV1): Promise<Uint8Array> {
  ...
  if (outline.title?.trim()) {
    const titleLines = wrapText(outline.title.trim(), CONTENT_WIDTH, fontBold, 20)
    ctx = drawLines(ctx, titleLines, 20, fontBold)
  }

  for (const block of outline.blocks) {
    if (block.type === 'heading') {
      const size = HEADING_SIZES[block.level]
      const lines = wrapText(block.text, CONTENT_WIDTH, fontBold, size)
      ctx = drawLines(ctx, lines, size, fontBold)
    } else if (block.type === 'paragraph') {
      const lines = wrapText(block.text, CONTENT_WIDTH, fontRegular, BODY_SIZE)
      ctx = drawLines(ctx, lines, BODY_SIZE, fontRegular, rgb(0.12, 0.14, 0.18))
...
export function defaultPdfFileNameFromOutline(outline: PdfOutlineV1): string {
  return sanitizePdfFileName(outline.fileName ?? outline.title)
}
