...
export function useChatPageCollaboration({
  ...
  /*
   * `isTemporary` heisst: der Thread ist noch nicht persistiert — ein frischer, ungeschriebener
   * Chat direkt nach "Neuer Chat" (siehe `useChat.ts`). Dort macht "Lernpfad erstellen" noch
   * keinen Sinn, es gibt ja noch nichts, woraus ein Lernpfad entstehen koennte.
   */
  const showLearningPathToolbarChip = Boolean(user && activeThread?.id && !activeThread.isTemporary)
