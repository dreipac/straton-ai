...
import {
  CHAT_WEB_SEARCH_MODE_STORAGE_KEY,
  type ChatWebSearchMode,
  parseStoredChatWebSearchMode,
...
export function useChat(
  ...
  /** Abo: gesperrte Modelle; leer = volle Auswahl (z. B. Gast). */
  ...
  options?: {
    ...
    /**
     * Nach jedem Turn das Profil aktualisieren — jeder Turn belastet die KI-Credits, nicht nur
     * Denken. Speist die Verbrauchsanzeige, ohne dass die Seite neu geladen werden muss.
     */
    onAiCreditsConsumed?: () => void | Promise<void>
  ...
  /** `null` = Smart Instant wählt das Modell selbst; sonst das fest gewählte Modell. */
  const [composerModelId, setComposerModelId] = useState<ChatComposerModelId | null>(() =>
    parseStoredComposerModelChoice(
  ...
  const [chatWebSearchMode, setChatWebSearchMode] = useState<ChatWebSearchMode>(() =>
    parseStoredChatWebSearchMode(
      typeof window !== 'undefined' ? localStorage.getItem(CHAT_WEB_SEARCH_MODE_STORAGE_KEY) : null,
    ),
  )
  ...
  function persistComposerModelId(id: ChatComposerModelId | null) {
    if (id && blockedModelIds.includes(id)) {
      return
    }
  ...
  function persistChatWebSearchMode(mode: ChatWebSearchMode) {
    setChatWebSearchMode(mode)
    try {
      localStorage.setItem(CHAT_WEB_SEARCH_MODE_STORAGE_KEY, mode)
    } catch {
      /* ignore */
    }
  }
  ...
  /** Eine Auswahl für das Composer-Menü — Verarbeitungsweg und Modell zusammen. */
  const composerSelection = toChatComposerSelection({
    thinkingMode: chatThinkingMode,
    modelId: composerModelId,
  })

  function persistComposerSelection(selection: ChatComposerSelection) {
    const next = fromChatComposerSelection(selection)
    if (next.modelId && blockedModelIds.includes(next.modelId)) {
      return
    }
    persistChatThinkingMode(next.thinkingMode)
    persistComposerModelId(next.modelId)
  }

  /* Sperrt das Abo nachträglich das gewählte Modell, fällt der Chat auf die automatische Wahl
     zurück — sonst bliebe eine Auswahl stehen, die serverseitig ohnehin abgelehnt würde. */
  const isSelectedModelBlocked = Boolean(composerModelId && blockedModelIds.includes(composerModelId))
  useEffect(() => {
    if (!isSelectedModelBlocked) {
      return
    }
    setComposerModelId(null)
    try {
      localStorage.removeItem(CHAT_COMPOSER_MODEL_STORAGE_KEY)
    } catch {
      /* ignore */
  ...
  /** `null` = keine feste Wahl; das Routing entscheidet dann selbst. */
  const effectiveComposerModelId: ChatComposerModelId | null = isSelectedModelBlocked
    ? null
    : composerModelId

  /**
   * Hat der Nutzer explizit ein Anthropic-Modell gewählt, soll auch die Instant-Einordnung auf
   * Anthropic laufen (günstiges Haiku) statt auf Gemini/OpenAI — kein Fremdanbieter-Aufruf pro
   * Turn mehr (Massnahme M4, straton-caching-fix-plan.md). Ohne feste Wahl bleibt es beim
   * bisherigen Gemini/OpenAI-Routing des Analyzers.
   */
  const explicitAnthropicChat = Boolean(
    effectiveComposerModelId && getChatComposerModelMeta(effectiveComposerModelId).provider === 'anthropic',
  )
    ...
    try {
      ...
      try {
        if (wantsInstantAnalyze) {
          ...
          const invokeResult = await instantAnalyzeUserMessage({
            ...
            folderContext,
            signal,
            preferAnthropicAnalyze: explicitAnthropicChat,
          })
          ...
          /* Ohne eigene Suchanfrage aus der Analyse dient die Nachricht selbst als Anfrage — nötig
             für «Ein», wo auch dann gesucht wird, wenn die Analyse keine Anfrage formuliert hat. */
          const webQuery = instantAnalyze.web_query.trim() || routingText.trim()
          /* Turns, die gar nicht antworten (Rückfrage) oder etwas erzeugen statt zu antworten
             (Dokument, Bild), bleiben auch bei «Ein» ohne Suche — dort gäbe es nichts zu belegen. */
          const canWeb =
          ...
          const shouldAutoWeb =
            chatWebSearchMode === 'off'
              ? false
              : canWeb && (chatWebSearchMode === 'on' || instantAnalyze.needs_live_web)
          ...
          if (shouldAutoWeb) {
            if (!options?.isSuperadmin && (options?.webSearchCreditBalance ?? 0) < 1) {
              ...
              try {
                const ws = await fetchTavilySearchContext(webQuery, signal)
      ...
      if (
        ...
        if (thinkingConversationPhase === 'clarify' && focusDim && progress && progress.roundsTotal > 0) {
          ...
          const thinkingWebQuery = thinkingAnalyzeResult.web_query.trim() || trimmed
          const shouldAutoWebThinking =
            chatWebSearchMode === 'off'
              ? false
              : thinkingWebQuery.length > 0 &&
                (chatWebSearchMode === 'on' || thinkingAnalyzeResult.needs_live_web)
          ...
          if (shouldAutoWebThinking) {
            if (!options?.isSuperadmin && (options?.webSearchCreditBalance ?? 0) < 1) {
              ...
              setSendPhase('web_search')
              try {
                const ws = await fetchTavilySearchContext(thinkingWebQuery, signal)
                webSearchContext = ws.contextText
                usedAutoWebSearch = true
                void options?.onWebSearchCreditsConsumed?.()?.catch(() => {})
              } catch (wsErr) {
          ...
          /* Die Prüfung des Entwurfs kann eine Suche nachfordern — bei «Aus» bleibt sie aussen vor. */
          if (
            chatWebSearchMode !== 'off' &&
            review.needs_live_web &&
            review.web_query.trim().length > 0 &&
            !usedAutoWebSearch
          ) {
            if (!options?.isSuperadmin && (options?.webSearchCreditBalance ?? 0) < 1) {
              setError(
      ...
      if (usesGatewayAi()) {
        ...
        try {
          ...
          if (
            ...
            streamErr.message.includes('KI-Credits-Guthaben')
          ) {
      ...
      if (
        ...
        err.message.includes('KI-Credits-Guthaben')
      ) {
        markThinkingCreditsDepletedLocally()
      }
      ...
      setLiveThinkingAnalyzeDebug(null)
      setIsSending(false)
      /* Jeder Turn bucht KI-Credits (`charge_ai_credits_usage`, Modus `chat`) — Smart Instant und
         eine feste Modellwahl genauso wie Denken, nur je nach Modell unterschiedlich viel. Der
         Abgleich holt den gebuchten Stand ins Profil; ohne ihn stünde die Verbrauchsanzeige bis
         zum nächsten Seitenaufruf still. Bewusst ohne `await`: die Eingabe soll nicht auf den
         Profil-Abruf warten. Auch Superadmins werden gebucht, also auch für sie. */
      if (!aborted) {
        void options?.onAiCreditsConsumed?.()?.catch(() => {})
      }
    }
  }
  ...
  return {
    ...
    composerSelection,
    setComposerSelection: persistComposerSelection,
    chatReplyMode,
    setChatReplyMode: persistChatReplyMode,
    chatThinkingMode,
    setChatThinkingMode: persistChatThinkingMode,
    chatWebSearchMode,
    setChatWebSearchMode: persistChatWebSearchMode,
