...
export type SendMessageOptions = {
   ...
   */
  /** Fest gewähltes Modell; `null`/fehlend = das Routing wählt selbst. */
  mainChatModelId?: ChatComposerModelId | null
  /**
...
/**
 * Hat die Person im Composer ein festes Modell gewählt? Dann gilt es für die Antwort und schlägt
 * jede automatische Modellwahl — auch die Sonderwege für Zusammenfassungen und PPTX-Bearbeitungen
 * und die Tages-Staffelung nach Tokenverbrauch. Wer ein Modell auswählt, bekommt dieses Modell.
 */
function pickedMainChatModel(options?: SendMessageOptions): ChatComposerModelId | null {
  if (options?.useLearnPathModel) {
    return null
  }
  return options?.mainChatModelId ?? null
}
    ...
    try {
      ...
      if (text) {
        try {
          ...
          if (parsed.error === 'AI_CREDITS_LIMIT' || parsed.error === 'THINKING_LIMIT') {
 ...
 *
 * Reihenfolge bewusst: echte Nutzerfrage ZUERST, Kontext-Block DANACH. Nur so ist die reine Frage
 * (das, was ohne Kontext in der DB liegt und in künftigen Turns als Verlauf repliziert wird) ein
 * PRÄFIX dessen, was hier gesendet wird — nicht ein Suffix. Stünde der Kontext vorne, würde jede
 * Nachricht beim ersten Senden anders aussehen als bei jedem späteren Replay als Verlauf, und ein
 * präfixbasierter Cache (OpenAI) könnte nie über den Systemprompt hinauswachsen (Befund 4,
 * straton-caching-fix-plan.md).
 */
...
function buildChatCompletionRequestBody(
  ...
  /* Thinking bringt eine eigene Modellkette mit; eine feste Wahl gilt nur für den Smart-Instant-Weg. */
  const pickedModelId = thinking ? null : pickedMainChatModel(options)
  const pickedModelMeta = pickedModelId ? getChatComposerModelMeta(pickedModelId) : null
  ...
  /* Alle Sonderwege stehen unter dem Vorbehalt `!pickedModelMeta`: Eine bewusste Modellwahl schlägt
     sie, sonst würde die Antwort trotz Auswahl auf einem anderen Modell landen. */
  ...
  const pptxEditOpenAi = !thinking && !pickedModelMeta && Boolean(options?.userRequestedPptxEdit)
  const categoryActionModel =
    !thinking && !pickedModelMeta && !summaryInstantOpenAi && options?.instantAnalyze
      ...
      : thinking
        ...
        : summaryInstantOpenAi
          ...
          : pptxEditOpenAi
            ...
            : pickedModelMeta ?? getChatComposerModelMeta('gpt-5.4-mini')
  const body: Record<string, unknown> = {
    mode: 'chat',
  ...
  if (pickedModelId) {
    /* Die Edge Function prüft die Wahl gegen die Sperrliste des Abos — im Client allein wäre sie
       nicht durchsetzbar. */
    body.chatModelId = pickedModelId
  }
    ...
    try {
      ...
      if (t) {
        try {
          ...
          if (j.error === 'AI_CREDITS_LIMIT' || j.error === 'THINKING_LIMIT') {
            throw new Error(
          ...
          if (parseErr instanceof Error && parseErr.message.includes('KI-Credits-Guthaben')) {
            throw parseErr
          }
          if (t.length < 600) {
            msg = t.trim()
      ...
      if (outer instanceof Error && outer.message.includes('KI-Credits-Guthaben')) {
        throw outer
      }
      /* ignore */
    }
...
export async function instantAnalyzeUserMessage(params: {
  ...
  signal?: AbortSignal
  /** Nutzer hat explizit ein Anthropic-Modell gewählt: Einordnung läuft auf Claude Haiku statt
   *  Gemini/OpenAI — kein Fremdanbieter-Aufruf pro Turn (Massnahme M4, straton-caching-fix-plan.md). */
  preferAnthropicAnalyze?: boolean
  ...
  try {
    ...
    const { data, error, response } = await supabase.functions.invoke('chat-completion', {
      body: {
        ...
        /* Explizite Anthropic-Modellwahl: Einordnung auf Claude Haiku statt Gemini/OpenAI (M4). */
        ...(params.preferAnthropicAnalyze ? { preferAnthropicAnalyze: true } : {}),
        payload: {
