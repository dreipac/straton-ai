...
export function ChatWindow({
  ...
  const [pendingAttachments, setPendingAttachments] = useState<
    PendingAttachment[]
  >([]);
  const [sentPastedImagePreviews, setSentPastedImagePreviews] = useState<
  ...
  const [animatedAssistantContent, setAnimatedAssistantContent] = useState<
    Record<string, string>
  >({});
  const [quizAnswers, setQuizAnswers] = useState<
    Record<string, QuizAnswerState>
  >({});
  const [quizChecksInProgress, setQuizChecksInProgress] = useState<
    Record<string, boolean>
  >({});
  const inputRef = useRef<HTMLTextAreaElement | null>(null);
  const fileInputRef = useRef<HTMLInputElement | null>(null);
  const [isAttachingFiles, setIsAttachingFiles] = useState(false);
  const [excelDownloadBusyId, setExcelDownloadBusyId] = useState<string | null>(
    null,
  );
  ...
  const isAssistantReplyStillAnimating = (() => {
    ...
    const animated =
      animatedAssistantContent[lastMessage.id] ?? lastMessage.content;
  ...
  useEffect(() => {
    ...
    for (const message of messages) {
      ...
      if (shouldStreamLatest && latestMessage?.id === message.id) {
        ...
        const durationMs = Math.min(
          900,
          Math.max(120, (fullContent.length / charsPerSecond) * 1000),
        );
    ...
    return () => {
      cancelRafChain();
      if (
        streamingIdForCleanup &&
        streamingAssistantIdsRef.current.has(streamingIdForCleanup)
      ) {
  ...
  async function downloadExcelExport(message: ChatMessage) {
    ...
    try {
      ...
      const { data, error } = await supabase.storage
        .from(ex.bucket)
        .createSignedUrl(ex.path, 3600);
      if (error || !data?.signedUrl) {
        throw new Error(
          error?.message ?? "Download-Link konnte nicht erstellt werden.",
        );
      }
  ...
  function updateQuizAnswerValue(
    messageId: string,
    questionId: string,
    value: string,
  ) {
  ...
  async function checkQuizAnswer(message: ChatMessage, questionId: string) {
    ...
    const question = parsed.quiz.questions.find(
    ...
    try {
      ...
      setQuizAnswers((prev) => ({
        ...
        [key]: {
          ...
          status: "incorrect",
          feedback:
            "KI Bewertung momentan nicht erreichbar. Bitte erneut prüfen.",
        },
      }));
    } finally {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const pastedImageEntries = pendingAttachments.filter(
      (
        entry,
      ): entry is PendingAttachment & {
        kind: "pasted-image";
        previewDataUrl: string;
      } =>
        entry.kind === "pasted-image" &&
        typeof entry.previewDataUrl === "string" &&
        entry.previewDataUrl.length > 0,
    );
  ...
  function extractPastedImageIdsFromContent(content: string): string[] {
    const regex =
      /\[(?:BildData|Bild):([^:\]]+)(?::[^\]]+)?\][\s\S]*?\[\/(?:BildData|Bild)\]/g;
      ...
      >
        {tokenLimitReached ? (
          <p className="chat-limit-banner" role="alert">
            Dein Token-Limit für heute ist erreicht. Du kannst morgen wieder
            schreiben.
          </p>
        ) : null}
        <div className="chat-empty-compose">
          <div className="chat-empty-leading-spacer" aria-hidden="true" />
          <div className="chat-empty-body">
            ...
            >
              ...
              <div className="chat-left-actions">
                ...
                >
                  <img
                    className="ui-icon chat-send-icon"
                    src={attachmentIcon}
                    alt=""
                    aria-hidden="true"
                  />
              ...
              <div className="chat-input-compose">
                {pendingAttachments.length > 0 ? (
                  <div
                    className="chat-attachment-chips"
                  ...
                  >
                    {pendingAttachments.map((item) =>
                      item.kind === "pasted-image" && item.previewDataUrl ? (
                        <span
                          key={item.id}
                          className="chat-attachment-chip chat-attachment-chip--image"
                        >
                          <img
                            className="chat-attachment-inline-preview"
                ...
                <div className="chat-input-field">
                  {showSlashMenu ? (
                    <div
                      className="chat-slash-menu thread-menu"
                      role="menu"
            ...
            <p className="chat-input-hint">
              Straton ist eine KI und kann Fehler machen, überprüfe wichtige
              Informationen
            </p>
          </div>
        </div>
  ...
  return (
    <section
      className={`chat-panel${tokenLimitReached ? " has-limit-banner" : ""}`}
    >
      {tokenLimitReached ? (
        <p className="chat-limit-banner" role="alert">
          Dein Token-Limit für heute ist erreicht. Du kannst morgen wieder
        ...
        <div className="chat-messages-inner">
          {messages.map((message) => {
            ...
            const parsed = isAssistant
              ? parseInteractiveContentWithFallback(message.content)
              : null;
            ...
            const baseAssistantForDisplay = message.metadata?.liveStream
              ...
              : message.metadata?.excelExport
                ? message.content
                : animatedContent;
            const rawAssistantDisplay = hasInteractiveQuiz
              ? parsed?.cleanText || ""
              : baseAssistantForDisplay;
            ...
            const pastedImageIds =
              message.role === "user"
                ? extractPastedImageIdsFromContent(message.content)
                : [];
            ...
            return (
              ...
              >
                ...
                {message.role === "user" && pastedImageIds.length > 0 ? (
                  <div
                    className="chat-user-inline-images"
                    aria-label="Eingefügte Bilder"
                  >
                ...
                {showExcelFallbackText ? (
                  <p className="chat-message-body chat-excel-fallback-text">
                    Die Excel-Datei ist bereit — nutze den Download-Button
                    unten.
                  </p>
                ) : null}
                ...
                {hasInteractiveQuiz ? (
                  <section
                    className="interactive-quiz-block"
                    aria-label="Interaktive Prüfungsfragen"
                  >
                    {parsed?.quiz?.title ? (
                      <h4 className="interactive-quiz-title">
                        {parsed.quiz.title}
                      </h4>
                    ) : null}

                    {parsed?.quiz?.questions.map((question) => {
                      const current = getQuizAnswerState(
                        message.id,
                        question.id,
                      );
                      ...
                      return (
                        <div
                          key={question.id}
                          className={`interactive-quiz-question ${statusClass}`}
                        >
                          <p className="interactive-quiz-prompt">
                            {question.prompt}
                          </p>
                          ...
                          {current.feedback ? (
                            <p
                              className={`interactive-quiz-feedback ${statusClass}`}
                            >
                              {current.feedback}
                            </p>
                          ) : null}
