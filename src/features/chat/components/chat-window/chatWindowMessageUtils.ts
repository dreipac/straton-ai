...
/**
 * Trennt einen Dateinamen in Basisname und Endung (ohne Punkt, Grossbuchstaben) — für die
 * Anhang-Pill, die die Endung dezent unter dem Namen statt inline im Namen zeigt.
 * Kein Punkt oder Punkt am Anfang (".gitignore") -> Endung leer, ganzer Name bleibt Basisname.
 */
export function splitFileNameForDisplay(name: string): { base: string; ext: string } {
  const dotIndex = name.lastIndexOf('.')
  if (dotIndex <= 0 || dotIndex === name.length - 1) {
    return { base: name, ext: '' }
  }
  return { base: name.slice(0, dotIndex), ext: name.slice(dotIndex + 1).toUpperCase() }
}
