...
import { parseUiSettings, type UiSettingsV1 } from '../../settings/constants/uiSettings'
import {
...
const PROFILE_SELECT =
  `first_name, last_name, avatar_url, auto_remove_empty_chats, auto_remove_empty_learning_paths, is_superadmin, language, chat_onboarding_completed, beta_notice_seen, ui_settings, must_change_password_on_first_login, subscription_plan_id, subscription_plans!subscription_plan_id ( id, name, max_tokens, instant_token_balance_max, max_images, max_files, image_generation_model, chat_blocked_model_ids, chat_daily_tier1_openai_model_id, chat_daily_tier1_token_budget, chat_daily_tier2_openai_model_id, chat_context_max_tokens, web_search_daily_grant, web_search_start_balance, web_search_credit_max, image_start_balance, image_credit_max, thinking_start_balance, thinking_daily_grant, thinking_credit_max, thinking_tier1_openai_model_id, thinking_tier1_token_budget, thinking_tier2_openai_model_id, ai_credits_daily_grant, ai_credits_start_balance, ai_credits_balance_max ), subscription_usages ( used_tokens, used_images, used_files, image_credit_balance, token_balance, web_search_credit_balance, used_web_searches, thinking_credit_balance, used_thinking_requests, last_reset_date, used_ai_credits_today, ai_credits_balance ), ${PROFILE_INTRODUCTION_SELECT}, ai_chat_memory, ai_chat_memory_enabled` as const
...
type ProfileRow = {
  ...
  subscription_usages:
    | {
        ...
        thinking_credit_balance?: number | null
        used_thinking_requests?: number | null
        last_reset_date: string | null
        used_ai_credits_today?: number | null
...
function mapProfileRow(data: ProfileRow | null): UserProfile | null {
  ...
  const plan =
    planRow == null
      ...
      : {
          ...planRow,
          image_generation_model: parseSubscriptionImageGenerationModelId(
            planRow.image_generation_model ?? null,
          ),
        }
  ...
  const thinkBal =
    typeof rawUsage?.thinking_credit_balance === 'number' ? rawUsage.thinking_credit_balance : 0
  const aiCreditsBal =
    typeof rawUsage?.ai_credits_balance === 'number' ? rawUsage.ai_credits_balance : 0
  const usage =
    rawUsage && rawUsage.last_reset_date !== currentUtcDateString()
      ? {
          ...
          thinking_credit_balance: thinkBal,
          ai_credits_balance: aiCreditsBal,
        }
      : rawUsage
        ? {
            ...
            used_thinking_requests:
              typeof rawUsage.used_thinking_requests === 'number' ? rawUsage.used_thinking_requests : 0,
            used_ai_credits_today:
              typeof rawUsage.used_ai_credits_today === 'number' ? rawUsage.used_ai_credits_today : 0,
            image_credit_balance: bal,
            token_balance: tokBal,
            web_search_credit_balance: wsBal,
            thinking_credit_balance: thinkBal,
...
export type UserProfile = {
  ...
  subscription_usages: {
    ...
    used_images: number
    used_files: number
    /** Verfügbare Bilder aus Guthaben; täglich +max_images vom Abo (UTC), gecappt mit image_credit_max. */
    ...
    /** Verfügbare Thinking-Modus-Anfragen; täglich +thinking_daily_grant (UTC). */
    ...
    /** Übertragenes KI-Credits-Guthaben (Carryover), gedeckelt mit ai_credits_balance_max des Abos. */
...
/**
 * Prüft, ob das übergebene Passwort das aktuelle ist. Supabase kennt keine reine Passwortprüfung,
 * deshalb läuft es über eine Anmeldung derselben Person — die Speicherwahl (localStorage vs.
 * sessionStorage) bleibt dabei bewusst unangetastet, es werden nur die Tokens erneuert.
 * Falsches Passwort → `false`; alles andere (Netzwerk, Rate-Limit) wirft.
 */
...
export async function updateAiChatMemoryByUserId(
  ...
  let payload = patch
  if (typeof patch.ai_chat_memory === 'string') {
    payload = {
      ...patch,
      ai_chat_memory: clipAiChatMemoryToMaxTokens(patch.ai_chat_memory),
    }
  }
