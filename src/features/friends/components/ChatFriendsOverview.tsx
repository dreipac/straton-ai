    ...
    >
      <div className="chat-friends-overview-inner">
        ...
        >
          ...
          <button
            type="button"
            className={`learn-top-tab learn-top-tab--invitations${tab === 'invitations' ? ' is-active' : ''}`}
            onClick={() => onTabChange('invitations')}
          >
            <span className="learn-top-tab-label">Einladungen</span>
          </button>
        </nav>

        <div key={tab} className="chat-friends-overview-tab-content">
          <div className="chat-friends-overview-panel learn-tab-panel">
            {/* Fehler-/Ladezustand gehören zu den Freundesdaten — der Einladungs-Tab lädt eigenständig. */}
            {error && tab !== 'invitations' ? (
              <p className="error-text chat-friends-overview-error">{error}</p>
            ) : null}
            {isLoading && tab !== 'invitations' ? (
              <p className="chat-friends-overview-empty">Wird geladen…</p>
            ) : null}
            ...
            {tab === 'invitations' ? <ChatInvitationsPanel userId={userId} /> : null}
          </div>
        </div>
      </div>
