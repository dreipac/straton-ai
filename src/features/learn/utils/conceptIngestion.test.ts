...
describe('parseConceptGraphFromText — Herkunft (Invariante I4)', () => {
  /*
   * I4: „Jedes Wissensatom traegt eine Herkunftsmarkierung." Der teure Fehler ist nicht die
   * fehlende Markierung, sondern die falsche: ein Konzept, das Materialherkunft behauptet, ohne
   * die Stelle nennen zu koennen. Genau das soll der Parser verhindern.
   */
  it('uebernimmt Materialherkunft nur mit woertlichem Beleg', () => {
    const g = parseConceptGraphFromText(
      JSON.stringify({
        concepts: [
          { slug: 'a', name: 'A', difficulty: 2, origin: 'material', quote: 'Ein Subnetz teilt ein Netz.' },
          { slug: 'b', name: 'B', difficulty: 2, origin: 'material' },
          { slug: 'c', name: 'C', difficulty: 2, origin: 'ai_supplement' },
          { slug: 'd', name: 'D', difficulty: 2 },
        ],
        edges: [],
      }),
    )

    const bySlug = new Map(g.concepts.map((c) => [c.slug, c]))
    expect(bySlug.get('a')).toMatchObject({ origin: 'material', sourceQuote: 'Ein Subnetz teilt ein Netz.' })
    // Behauptet Material, nennt aber keine Stelle -> heruntergestuft statt geglaubt.
    expect(bySlug.get('b')).toMatchObject({ origin: 'ai_supplement', sourceQuote: '' })
    expect(bySlug.get('c')?.origin).toBe('ai_supplement')
    // Gar keine Angabe ist derselbe Fall: nie stillschweigend zu 'material' werden.
    expect(bySlug.get('d')?.origin).toBe('ai_supplement')
  })

  it('verwirft den Beleg, wenn die Herkunft nicht material ist', () => {
    const g = parseConceptGraphFromText(
      JSON.stringify({
        concepts: [{ slug: 'a', name: 'A', difficulty: 2, origin: 'ai_supplement', quote: 'irgendwas' }],
        edges: [],
      }),
    )
    expect(g.concepts[0]).toMatchObject({ origin: 'ai_supplement', sourceQuote: '' })
  })
})

describe('mergeConceptGraphs — Herkunft', () => {
  it('ein Beleg gewinnt gegen keinen Beleg', () => {
    const ergaenzt: IngestedGraph = {
      concepts: [{ slug: 'x', name: 'X', description: '', difficulty: 2, sourceRef: {}, origin: 'ai_supplement', sourceQuote: '' }],
      edges: [],
    }
    const belegt: IngestedGraph = {
      concepts: [{ slug: 'x', name: 'X', description: '', difficulty: 2, sourceRef: {}, origin: 'material', sourceQuote: 'Beleg aus Kapitel 2.' }],
      edges: [],
    }

    expect(mergeConceptGraphs([ergaenzt, belegt]).concepts[0]).toMatchObject({
      origin: 'material',
      sourceQuote: 'Beleg aus Kapitel 2.',
    })
    // Und nicht in die andere Richtung: ein vorhandener Beleg wird nie ueberschrieben.
    expect(mergeConceptGraphs([belegt, ergaenzt]).concepts[0]).toMatchObject({
      origin: 'material',
      sourceQuote: 'Beleg aus Kapitel 2.',
    })
  })
...
describe('mergeConceptGraphs', () => {
  it('dedupliziert Konzepte per Slug: längere Beschreibung + höchste Schwierigkeit gewinnen', () => {
    const a: IngestedGraph = {
      concepts: [{ slug: 'x', name: 'X', description: 'kurz', difficulty: 2, sourceRef: { section: 'A', pageFrom: 3 }, origin: 'ai_supplement' as const, sourceQuote: '' }],
      edges: [],
    }
    const b: IngestedGraph = {
      concepts: [
        { slug: 'x', name: 'X lang', description: 'eine viel längere beschreibung', difficulty: 4, sourceRef: { pageTo: 7 }, origin: 'ai_supplement' as const, sourceQuote: '' },
        { slug: 'y', name: 'Y', description: '', difficulty: 3, sourceRef: {}, origin: 'ai_supplement' as const, sourceQuote: '' },
      ],
      edges: [],
    }
  ...
  it('vereinigt Kanten und filtert auf überlebende Slugs, ohne Doppel-/Selbstkanten', () => {
    const a: IngestedGraph = {
      concepts: [
        { slug: 'a', name: 'A', description: '', difficulty: 1, sourceRef: {}, origin: 'ai_supplement' as const, sourceQuote: '' },
        { slug: 'b', name: 'B', description: '', difficulty: 1, sourceRef: {}, origin: 'ai_supplement' as const, sourceQuote: '' },
      ],
      edges: [{ fromSlug: 'a', toSlug: 'b', type: 'prerequisite' }],
    }
    const b: IngestedGraph = {
      concepts: [{ slug: 'b', name: 'B', description: '', difficulty: 1, sourceRef: {}, origin: 'ai_supplement' as const, sourceQuote: '' }],
      edges: [
  ...
  it('begrenzt die Gesamtzahl der Konzepte auf CONCEPT_INGESTION_MAX_CONCEPTS (Dokumentreihenfolge zuerst)', () => {
    const many: IngestedGraph = {
      concepts: Array.from({ length: CONCEPT_INGESTION_MAX_CONCEPTS + 10 }, (_, i) => ({
        ...
        description: '',
        difficulty: 3,
        sourceRef: {}, origin: 'ai_supplement' as const, sourceQuote: '',
      })),
      edges: [],
    }
