...
/**
 * Wird VOR jedem `createLearningPathByUserId` geprüft — eigenes monatliches Kontingent statt
 * Kopplung an das tägliche KI-Credits-Guthaben (siehe `checkLearningPathCreationAllowed` und
 * AI_CREDITS_EXCLUDED_MODES in der Edge Function `chat-completion`). Bei Fehlern der RPC selbst
 * (statt eines echten "Kontingent voll") wird nicht blockiert — reiner Read-Check, kein Datenverlust
 * beim Durchlassen.
 */
async function checkLearningPathCreationAllowed(userId: string): Promise<boolean> {
  const supabase = getSupabaseClient()
  const { data, error } = await supabase.rpc('check_learning_path_creation_allowed', {
    p_user_id: userId,
  })
  if (error) {
    return true
...
/** Autoritative Buchung nach erfolgreichem Anlegen — siehe `checkLearningPathCreationAllowed`. */
async function chargeLearningPathCreation(userId: string): Promise<void> {
  const supabase = getSupabaseClient()
  const { error } = await supabase.rpc('charge_learning_path_creation', { p_user_id: userId })
  if (error) {
    console.warn('[learn] charge_learning_path_creation:', error.message)
  }
}
...
export async function createLearningPathByUserId(
  ...
  // Platzhalter-Modus verursacht keine KI-Kosten (Admin-Test) -> zaehlt nicht gegen das
  // Monatskontingent, genau wie er auch nicht gegen das taegliche KI-Credits-Guthaben zaehlt.
  const countsTowardQuota = generationMode !== 'placeholder'

  if (countsTowardQuota) {
    const allowed = await checkLearningPathCreationAllowed(userId)
    if (!allowed) {
      throw new Error(
        'Du hast dein monatliches Lernpfad-Kontingent erreicht. Es wird am 1. jedes Monats zurückgesetzt.',
      )
    }
  }
  ...
  if (countsTowardQuota) {
    void chargeLearningPathCreation(userId)
  }
