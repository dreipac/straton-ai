...
type LearnEntryQuizMatchProps = {
  ...
  /** Bedienhinweis ("Ziehe die rechten Karten …") ausblenden. Default false — bestehende
   *  Aufrufstellen im alten Lernmotor bleiben dadurch unveraendert. */
  hideHint?: boolean
  /** Zusaetzliche Klasse auf dem Wurzelelement — fuer Aufrufstellen-spezifisches CSS (z. B.
   *  `.brain-session-match` in `learn.css`), ohne die Basisklasse fuer alle Aufrufstellen
   *  mitzuaendern. */
  className?: string
}
...
export function LearnEntryQuizMatch(props: LearnEntryQuizMatchProps) {
  const { questionId, matchLeft, matchRight, value, disabled, onChange, hideHint = false, className } = props
  ...
  return (
    <div
      className={`learn-entry-test-match${className ? ` ${className}` : ''}`}
