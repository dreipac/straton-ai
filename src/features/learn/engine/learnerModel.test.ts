...
describe('applyConceptObservation — Propagation', () => {
  it('zieht verwandte Konzepte mit Zustand mit, erfindet aber keine Zustaende', () => {
    const a = concept('a')
    ...
    const { propagated } = applyConceptObservation({ concept: a, edges, statesById, correct: true, nowIso: NOW })
    // Nur b hat einen Zustand -> nur b wird propagiert (c wird nicht erfunden).
    expect(propagated).toHaveLength(1)
    expect(propagated[0].conceptId).toBe('b')
    expect(propagated[0].pMastery).toBeGreaterThan(0.5)
  })
})
