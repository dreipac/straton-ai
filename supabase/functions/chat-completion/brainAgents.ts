/**
 * Straton Gehirn — serverseitige Aufloesung der Modellrollen (Kapitel 12).
 *
 * Der Client sendet NUR die Rolle, nie ein Modell. Hier wird sie gegen
 * `learn_brain_agent_models` aufgeloest. Zwei Gruende, warum die Aufloesung serverseitig liegt:
 *
 *  1. Ein manipulierter Client kann sich kein teureres Modell erschleichen.
 *  2. Ein Modellwechsel im Admin-Menue wirkt sofort fuer alle, ohne dass ein Frontend neu
 *     ausgeliefert werden muss. Genau das meint Kapitel 12 mit „Ein Modellwechsel ist eine
 *     Konfigurationsaenderung, kein Umbau."
 *
 * Die Systemanweisungen der Rollen sind eine Kopie von
 * `src/features/learn/brain/agents/prompts.ts`. Sie liegen doppelt vor, weil Edge Functions
 * nicht in den Frontend-Build hineinreichen. Aendert sich eine Anweisung, muessen BEIDE Seiten
 * nachgezogen und der jeweilige Cache-Schluessel hochgezaehlt werden.
 */
...
export type BrainAgentBinding = {
  role: BrainRole
  provider: BrainProvider
  model: string
  escalationProvider: BrainProvider | null
...
export function isBrainRole(value: unknown): value is BrainRole {
  return typeof value === 'string' && (BRAIN_ROLES as readonly string[]).includes(value)
}

/**
 * Notbelegung, identisch zum Seed der Migration und zu `FALLBACK_BINDINGS` im Frontend.
 *
 * Bewusst ohne Anthropic: das Gehirn soll auch laufen, wenn nur OPENAI_API_KEY und
 * GEMINI_API_KEY gesetzt sind.
 */
...
function normaliseProvider(value: unknown): BrainProvider {
...
/**
 * Die Bindung einer Rolle laden.
 *
 * Faellt die Abfrage aus, greift die Notbelegung. Ein Gehirn, das bei einer fehlgeschlagenen
 * Konfigurationsabfrage stehenbleibt, waere fragiler als noetig — die Rolle laeuft dann eben
 * auf dem Standardmodell statt gar nicht.
 */
export async function fetchBrainAgentBinding(
  admin: AdminClient | null,
  role: BrainRole,
): Promise<BrainAgentBinding> {
  if (!admin) {
  ...
  try {
    const { data, error } = await admin
      .from('learn_brain_agent_models')
      .select('role, provider, model, escalation_provider, escalation_model, max_output_tokens')
      .eq('role', role)
      .maybeSingle()
    ...
    const model = typeof row.model === 'string' && row.model.trim() ? row.model.trim() : FALLBACK[role].model
    const escalationModel =
      typeof row.escalation_model === 'string' && row.escalation_model.trim()
        ? row.escalation_model.trim()
        : null

    return {
      role,
      provider: normaliseProvider(row.provider),
      model,
      escalationProvider: escalationModel ? normaliseProvider(row.escalation_provider) : null,
      escalationModel,
      maxOutputTokens:
        Number.isFinite(Number(row.max_output_tokens)) && Number(row.max_output_tokens) > 0
          ? Math.min(32768, Math.max(256, Math.floor(Number(row.max_output_tokens))))
          : FALLBACK[role].maxOutputTokens,
    }
  } catch {
    return FALLBACK[role]
  }
...
/**
 * Die tatsaechliche Modellwahl fuer einen Aufruf.
 *
 * Kapitel 5.3: das schnelle, guenstige Modell erledigt den Normalfall, das teure wird nur bei
 * Zweifel geweckt. Ist keine Eskalation konfiguriert, bleibt es beim Hauptmodell — der Client
 * bekommt ueber `escalated: false` mit, dass sein Wunsch nicht erfuellt werden konnte, und die
 * Wahrnehmungsschicht daempft die Bewertung dann ueber die niedrige Zuversicht.
 */
export function modelForBrainCall(
  binding: BrainAgentBinding,
  wantsEscalation: boolean,
): { provider: BrainProvider; model: string; escalated: boolean } {
  if (wantsEscalation && binding.escalationProvider && binding.escalationModel) {
    return { provider: binding.escalationProvider, model: binding.escalationModel, escalated: true }
  }
  return { provider: binding.provider, model: binding.model, escalated: false }
}

// ---------------------------------------------------------------------------
// Systemanweisungen — Kopie von src/features/learn/brain/agents/prompts.ts
// ---------------------------------------------------------------------------
...
const SYSTEM_PROMPTS: Record<BrainRole, string> = {
  kartograf: `${COMMON_HEADER}
...
export function brainSystemPrompt(role: BrainRole): string {
  return SYSTEM_PROMPTS[role]
}

/**
 * Prompt-Cache-Schluessel je Rolle.
 *
 * Die Version MUSS hochgezaehlt werden, wenn sich die zugehoerige Systemanweisung aendert.
 * Sonst liefert der Cache die alte Anweisung zu einem neuen Auftrag aus — ein Fehler, der sich
 * als unerklaerliches Rollenverhalten aeussert und schwer zu finden ist.
 */
...
/**
 * Den Auftrag in eine Nutzernachricht giessen.
 *
 * Bewusst JSON: der Auftrag ist strukturiert, und ein Modell, das strukturierten Input bekommt,
 * liefert eher strukturierten Output. Ein in Prosa gegossener Auftrag waere fuer das Modell
 * angenehmer und fuer die Auswertung schlechter.
 */
export function brainUserMessage(role: BrainRole, input: unknown): string {
  const label: Record<BrainRole, string> = {
    ...
    konsolidierer: 'Beobachtungen und bestehende Muster',
    erklaerer: 'Entwurfssatz',
  }
