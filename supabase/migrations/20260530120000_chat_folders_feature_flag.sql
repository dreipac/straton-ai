-- Chat-Ordner global per Admin ein-/ausschalten

alter table public.app_feature_flags
  add column if not exists chat_folders_enabled boolean not null default true;
...
create or replace function public.admin_set_chat_folders_enabled(p_enabled boolean)
returns void
language plpgsql
security definer
set search_path = public
...
begin
  ...
  if not caller_is_superadmin then
    raise exception 'Nur Superadmins duerfen Chat-Ordner global schalten.';
  end if;

  update public.app_feature_flags
  set
    chat_folders_enabled = coalesce(p_enabled, false),
    updated_at = now()
  where id = 1;
...
grant execute on function public.admin_set_chat_folders_enabled(boolean) to authenticated;
