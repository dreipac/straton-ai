-- Smart-Instant-Debug im Chat (nur Superadmin + Schalter im Admin Center)

alter table public.app_feature_flags
  add column if not exists instant_analyze_debug_enabled boolean not null default false;
...
create or replace function public.admin_set_instant_analyze_debug_enabled(p_enabled boolean)
returns void
language plpgsql
security definer
set search_path = public
...
begin
  ...
  if not caller_is_superadmin then
    raise exception 'Nur Superadmins duerfen den Instant-Analyse-Debug schalten.';
  end if;

  update public.app_feature_flags
  set
    instant_analyze_debug_enabled = coalesce(p_enabled, false),
    updated_at = now()
  where id = 1;
...
grant execute on function public.admin_set_instant_analyze_debug_enabled(boolean) to authenticated;
